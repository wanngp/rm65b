...
LEFT_OBSERVER = (0.260, -0.580, 0.780, 0.100, 0.720, 0.200)

RIGHT_HOME = (0.000, -0.350, 0.650, 0.000, 0.900, 0.000)
RIGHT_READY = (-0.280, -0.540, 0.760, -0.120, 0.740, -0.220)
RIGHT_APPROACH = (-0.560, -0.705, 1.010, -0.355, 0.525, -0.520)
RIGHT_PRESS = (-0.780, -0.780, 1.100, -0.500, 0.440, -0.680)
RIGHT_DEEP_PRESS = (-0.820, -0.805, 1.125, -0.528, 0.420, -0.708)
...
class Day02ForcePressDemo(Node):
    def __init__(self, args: argparse.Namespace) -> None:
        super().__init__("rm65b_day02_force_press_demo")
        self.args = args
        ...
        self.left_hold = hold_trajectory(LEFT_OBSERVER)
        self.right_press = make_trajectory(
            RIGHT_PRESS_CYCLE,
            args.cycle_duration,
    ...
    def publish_trajectory(self) -> None:
        if self.args.direct_gz:
            self.publish_direct_gz("/model/left_rm65b/joint_trajectory", self.left_hold)
            self.publish_direct_gz("/model/right_rm65b/joint_trajectory", self.right_press)
        deadline = time.monotonic() + 1.0
        while time.monotonic() < deadline and rclpy.ok():
            self.left_pub.publish(self.left_hold)
            self.right_pub.publish(self.right_press)
            rclpy.spin_once(self, timeout_sec=0.02)
            time.sleep(0.08)
        self.get_logger().info(
            f"published Day2 press cycle: points={len(self.right_press.points)} "
    ...
    def run(self) -> None:
        ...
        self.get_logger().info("Day2 gripper closed and held for the full force-press demo")
        ...
        self.publish_gripper(self.args.closed_position)
        self.get_logger().info("Day2 force press demo finished")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--duration", type=float, default=180.0, help="seconds; use 0 to run until Ctrl+C")
    parser.add_argument("--cycle-duration", type=float, default=12.0)
