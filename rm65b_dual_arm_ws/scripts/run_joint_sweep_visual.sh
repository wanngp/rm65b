...
DURATION="${1:-180}"
DOMAIN_ID="${2:-42}"
OUT_ROOT="${3:-$HOME/rm65b_joint_sweep_visual_$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="$OUT_ROOT"
...
export GZ_PARTITION="${GZ_PARTITION:-rm65b_joint_sweep_${DOMAIN_ID}_$$}"
...
WORLD_RUNTIME="$OUT_DIR/logs/rm65b_joint_sweep_world.sdf"
...
SDF_CONTROLLED="$OUT_DIR/logs/rm_65_joint_sweep_controlled.sdf"
...
python3 "$SCRIPT_DIR/generate_day_world.py" \
  ...
  --notes "$OUT_DIR/logs/day01_world_notes.txt"
...
if [[ -n "${DISPLAY:-}" || -n "${WAYLAND_DISPLAY:-}" ]]; then
  ...
  PIDS+=("$!")
  echo "Gazebo GUI opened. Demo will run for ${DURATION}s; close the window or Ctrl+C to stop early."
else
  echo "DISPLAY/WAYLAND_DISPLAY is not set; Gazebo GUI was not opened." | tee "$OUT_DIR/logs/gz_gui.log"
fi

sleep "${DEMO_START_DELAY:-3}"
python3 "$SCRIPT_DIR/joint_sweep_demo.py" \
  --duration "$DURATION" \
  > "$OUT_DIR/logs/joint_sweep_demo.log" 2>&1

echo "Joint sweep visual demo complete. Logs are under: $OUT_DIR/logs"
