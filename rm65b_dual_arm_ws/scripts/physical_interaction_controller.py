...
class PhysicalInteractionController:
    ...
    def update(self, progress: float) -> None:
        day = self.day
        if day in {"day01", "d1"}:
            self.fire_at("d1_close_both", progress, 0.10, lambda: self.set_gripper(0.004, "d1_gripper_close_both"))
            self.fire_at("d1_open_both", progress, 0.24, lambda: self.set_gripper(0.018, "d1_gripper_open_both"))
            self.fire_at(
                "d1_close_for_sync_motion",
                progress,
                0.52,
                lambda: self.set_gripper(0.004, "d1_gripper_close_during_dual_motion"),
            )
            self.fire_at(
                "d1_final_open",
                progress,
                0.78,
                lambda: self.set_gripper(0.018, "d1_gripper_final_open"),
            )
        elif day in {"day02", "d2"}:
            self.fire_at("d2_close", progress, 0.32, lambda: self.set_gripper(0.006, "d2_contact_stiff_grip"))
            self.fire_at("d2_open", progress, 0.83, lambda: self.set_gripper(0.018, "d2_release"))
        elif day in {"day04", "d4"}:
            self.fire_at("d4_hook_close", progress, 0.18, lambda: self.set_gripper(0.004, "d4_hook_yarn_close"))
            ...
            self.fire_at("d4_lift_hold", progress, 0.36, lambda: self.set_gripper(0.004, "d4_lift_yarn_hold"))
            self.fire_at("d4_pull_hold", progress, 0.58, lambda: self.set_gripper(0.004, "d4_pull_tight_hold"))
            self.fire_at("d4_exchange_relax", progress, 0.82, lambda: self.set_gripper(0.012, "d4_exchange_relax"))
            ...
            self.fire_at("d4_open", progress, 0.93, lambda: self.set_gripper(0.018, "d4_release_open"))
        elif day in {"day05", "d5"}:
            self.fire_at("d5_force_close", progress, 0.30, lambda: self.set_gripper(0.006, "d5_force_contact_close"))
            self.fire_at("d5_pick_close", progress, 0.45, lambda: self.set_gripper(0.004, "d5_yarn_pick_close"))
            ...
            self.fire_at("d5_open", progress, 0.88, lambda: self.set_gripper(0.018, "d5_release_open"))
        else:
            self.fire_at("d3_open", progress, 0.02, lambda: self.set_gripper(0.018, "d3_vision_open"))

    def run(self) -> None:
        self.log("physical_interaction_controller started; dynamic set_pose disabled")
        self.set_gripper(0.018, "initial_open")
