...
if [[ "${ALLOW_NON_MOVEIT_DAY03_VISUAL:-0}" != "1" ]]; then
  if [[ -f "$SCRIPT_DIR/run_day_visual.sh" ]]; then
    echo "run_day03_vision_visual.sh is a direct non-MoveIt diagnostic entry."
    echo "Delegating to the required MoveIt visual path: run_day_visual.sh day03"
    exec bash "$SCRIPT_DIR/run_day_visual.sh" day03 "$DOMAIN_ID" "$OUT_ROOT"
  fi
  echo "run_day03_vision_visual.sh direct mode is disabled because D1-D5 must use MoveIt." >&2
  echo "Missing $SCRIPT_DIR/run_day_visual.sh; run scripts/bootstrap_vmware_ubuntu.sh --all or use the current workspace." >&2
  echo "Set ALLOW_NON_MOVEIT_DAY03_VISUAL=1 only for direct visual-servo diagnostics." >&2
  exit 2
fi
